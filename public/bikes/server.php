...
function addMotorcycle($brand, $model, $cylinder, $prod_year, $plate): void
{
    global $conn;
    try {
        $stmt = $conn->query("INSERT INTO motorcycle (brand, model, cylinder, prod_year, plate, Id_checkride_user) VALUES (?, ?, ?, ?, ?, 1)");
        $stmt->execute([$brand, $model, $cylinder, $prod_year, $plate]);
        header('Location: accueil.php?success=Motorcycle added successfully.');
        exit();
    } catch (Exception $e) {
...
function updateMotorcycle($id, $brand, $model, $cylinder, $prod_year, $plate): void
{
    ...
    try {
        ...
        header('Location: accueil.php?success=Motorcycle updated successfully.');
        exit();
    } catch (Exception $e) {
...
function deleteMotorcycle($id): void
{
    ...
    try {
        ...
        $stmt->execute([$id]);
        header('Location: accueil.php?success=Motorcycle deleted successfully.');
        exit();
    } catch (Exception $e) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action == 'insert') {
        if (empty($brand) || empty($model) || empty($cylinder) || empty($prod_year) || empty($plate)) {
            header('Location: accueil.php?error=All fields are required.');
            exit();
        }
        ...
        if (empty($id)) {
            header('Location: accueil.php?error=ID is required.');
            exit();
        }
        deleteMotorcycle($id);
    } elseif ($action == 'edit') {
