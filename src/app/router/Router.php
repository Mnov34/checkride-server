...
/**
 * Router Class
 * Prends les URI et appele la méthode désirée
 */
class Router {
    private $routes = [];
    private $params = [];
    ...
    /**
     * Charge le fichier routes.php
     *
     * @param string $file Chemin vers le fichier
     * @return Router      L'instance du routeur, pour pouvoir utiliser ses methode
     */
    ...
    /**
     * Ajoute une route
     * Converti la route (string) en expression regulière pour un routeur plus flexible
     *
     * @param string $route Route string, la route complète ou en expression régulière
     * @param array $params Garde le controller et l'action de chaque route (si non nulle)
     * @return void
     */
    public function addRoute(string $route, array $params = []): void {
        // enlève les slash arrière
        $route = preg_replace('/\//', '\\/', $route);

        // Converti les variables
        $route = preg_replace('/\{([a-z0-9]+)\}/', '(?P<\1>[a-z0-9-]+)', $route);

        // Converti les variables avec des expression régulière (exemple {id:\d+} pour les chiffres)
        $route = preg_replace('/\{([a-z0-9]+):([^\}]+)\}/', '(?P<\1>\2)', $route);

        // Ajoute le "/" de commencement et de fin
        $route = '/^' . $route . '$/i';
